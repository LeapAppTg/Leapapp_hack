...
import { Quest, QuestObjective, QuestStatus, QuestType, isLinkQuestObjective, isUserMilestoneQuestObjective } from "@types";
    ...
    return (
        <div className={classJoiner(
            ...
            <div className={FlexGapColumn4AlignFlexStart.update({ fillFullWidth: true }).className}>
                ...
                <QuestProgress objective={objective}/>
            </div>
            <QuestAction showLoader={showLoader} claimQuest={claimQuest} objective={objective} status={status}/>
        </div>
    )
}

const QuestProgress: FC<{objective: QuestObjective}> = ({ objective }) => {
    if (!isUserMilestoneQuestObjective(objective)) return null
    return (
...
const QuestAction: FC<{showLoader: boolean, status: QuestStatus, objective: QuestObjective, claimQuest: () => any}> = ({ showLoader, status, objective, claimQuest }) => {
    if (showLoader) return <LoaderIcon/>
    if (status === QuestStatus.Claimed) return <CircleIconWrapper color={CircleIconWrapperColor.Green600} icon={CheckmarkIcon}/>
    if (isLinkQuestObjective(objective)) return <IconBox icon={ArrowIcon} size={IconSize.MediumBig}/>
    if (isUserMilestoneQuestObjective(objective)) {
        if (objective.goal > objective.progress) {
            if (objective.type === QuestType.ReferralsMilestone) return <IconBox icon={ArrowIcon} size={IconSize.MediumBig}/>
            return (
